...
export default function Sidebar() {
  ...
  return (
    <nav className="flex flex-col w-20 flex-shrink-0 border-r"
      style={{ background: 'var(--crm-surface)', borderColor: 'var(--crm-border)' }}>
      <div className="flex items-center justify-center py-5 border-b" style={{ borderColor: 'var(--crm-border)' }}>
        <img src="/summit-logo.png" alt="" className="w-8 h-8 opacity-70" onError={(e) => {
          e.target.style.display = 'none';
          e.target.parentElement.innerHTML = '<span style="color: var(--crm-accent); font-weight: 700; font-size: 22px;">S</span>';
        }} />
      </div>
      <div className="flex-1 py-3 flex flex-col items-center gap-1.5 overflow-y-auto">
        {navItems.map((item, i) => {
          if (item.type === 'divider') {
            return <div key={i} className="w-8 h-px my-2" style={{ background: 'var(--crm-border)' }} />;
          }
          ...
          return (
            <Link key={item.href} href={item.href}
              ...
              style={{
                ...
                background: active ? 'rgba(30, 144, 255, 0.12)' : 'transparent',
              }}>
              {icons[item.icon]}
              <span className="absolute left-full ml-3 px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50"
