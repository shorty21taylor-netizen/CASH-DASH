  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold">Market Profits</h1>
          <p className="text-[14px] mt-1" style={{ color: 'var(--crm-text-muted)' }}>Trading & investment gains</p>
        </div>
        <button onClick={() => setShowForm(true)} className="px-5 py-2.5 rounded-xl text-[14px] font-semibold" style={{ background: 'var(--crm-accent)', color: '#fff' }}>+ Log profit</button>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="glass-card-solid p-6 space-y-5">
          <h2 className="text-[15px] font-bold">Log Market Profit/Loss</h2>
          <div className="grid grid-cols-3 gap-5">
            <div>
              <label className="block text-[14px] mb-1.5" style={{ color: 'var(--crm-text-secondary)' }}>Source / broker</label>
              <input value={form.source} onChange={(e) => setForm({ ...form, source: e.target.value })} placeholder="e.g. Robinhood, TD" className="input-field" />
            </div>
            <div>
              <label className="block text-[14px] mb-1.5" style={{ color: 'var(--crm-text-secondary)' }}>Ticker / asset</label>
          ...
          <div className="grid grid-cols-3 gap-5">
            <div>
              <label className="block text-[14px] mb-1.5" style={{ color: 'var(--crm-text-secondary)' }}>Type</label>
      ...
      {chartData.length > 1 && (
        <TrendChart data={chartData} lines={[{ key: 'profit', name: 'Market P&L', color: '#1E90FF' }]} title="Monthly market P&L" />
      )}
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b" style={{ borderColor: 'var(--crm-border)' }}>
              ...
              <th className="text-left px-5 py-4 text-[14px] font-medium" style={{ color: 'var(--crm-text-secondary)' }}>Ticker</th>
              <th className="text-left px-5 py-4 text-[14px] font-medium" style={{ color: 'var(--crm-text-secondary)' }}>Source</th>
              <th className="text-right px-5 py-4 text-[14px] font-medium" style={{ color: 'var(--crm-text-secondary)' }}>Amount</th>
              <th className="text-left px-5 py-4 text-[14px] font-medium" style={{ color: 'var(--crm-text-secondary)' }}>Type</th>
          ...
          <tbody>
            {profits.map((p) => (
              <tr key={p.id} className="border-b hover:bg-white/[0.02]" style={{ borderColor: 'var(--crm-border)' }}>
                <td className="px-5 py-4 text-[14px]">{p.date}</td>
                <td className="px-5 py-4 text-[14px] font-semibold">{p.ticker || '—'}</td>
                <td className="px-5 py-4 text-[14px]" style={{ color: 'var(--crm-text-muted)' }}>{p.source || '—'}</td>
                <td className="px-5 py-4 text-right text-[14px] font-semibold" style={{ color: (p.amount || 0) >= 0 ? 'var(--crm-positive)' : 'var(--crm-negative)' }}>
                ...
                <td className="px-5 py-4">
                  <span className="text-[13px] px-3 py-1 rounded-lg font-medium" style={{
                    background: p.type === 'realized' ? 'rgba(30,144,255,0.12)' : 'rgba(255,255,255,0.05)',
                    color: p.type === 'realized' ? 'var(--crm-accent)' : 'var(--crm-text-secondary)',
                  }}>{p.type}</span>
                </td>
                <td className="px-5 py-4 text-[13px] truncate max-w-[200px]" style={{ color: 'var(--crm-text-muted)' }}>{p.notes}</td>
                <td className="px-5 py-4 text-right"><button onClick={() => handleDelete(p.id)} className="text-[13px]" style={{ color: 'var(--crm-negative)' }}>x</button></td>
              </tr>
            ))}
          </tbody>
        </table>
