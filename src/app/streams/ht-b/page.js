...
export default function HighTicketBPage() {
  ...
  function load() {
    fetch('/api/commissions?stream=htB')
      .then((r) => r.json())
      .then((d) => {
  ...
  async function handleSubmit(data) {
    await fetch('/api/commissions', {
      ...
      body: JSON.stringify({ ...data, stream: 'htB' }),
    });
    setShowForm(false);
    load();
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">High Ticket Offer B</h1>
          <p className="text-dark-400 text-sm">Flat {(DEFAULT_RATES.htB * 100).toFixed(0)}% commission rate</p>
        </div>
      ...
      {showForm && (
        <div className="bg-dark-800 border border-dark-700 rounded-xl p-6">
          <CommissionForm
            initial={{ stream: 'htB', client_name: '', amount: '', rate: DEFAULT_RATES.htB, status: 'pending', date: new Date().toISOString().split('T')[0] }}
        ...
        <table className="w-full">
          ...
          <tbody>
            ...
            {commissions.length === 0 && (
              <tr><td colSpan="5" className="px-4 py-8 text-center text-dark-400">No commissions yet for High Ticket B.</td></tr>
            )}
          </tbody>
        </table>
      </div>
