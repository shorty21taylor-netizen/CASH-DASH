    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[14px] mt-1" style={{ color: 'var(--crm-text-muted)' }}>Policy management & renewal projections</p>
        </div>
        <button onClick={() => { setForm(getBlank()); setEditing(null); setShowForm(true); }} className="px-5 py-2.5 rounded-xl text-[14px] font-semibold" style={{ background: 'var(--crm-accent)', color: '#fff' }}>
          + New policy
        </button>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="glass-card-solid p-6 space-y-5">
          <h2 className="text-[15px] font-bold">{editing ? 'Edit' : 'New'} Policy</h2>
          <div className="grid grid-cols-2 gap-5">
            ...
            <div>
              <label className="block text-[14px] mb-1.5" style={{ color: 'var(--crm-text-secondary)' }}>Sold date</label>
              <input name="sold_date" type="date" value={form.sold_date} onChange={handleChange} className="input-field" />
            </div>
          </div>
          <div className="grid grid-cols-4 gap-5">
            <div>
              <label className="block text-[14px] mb-1.5" style={{ color: 'var(--crm-text-secondary)' }}>Annual premium ($)</label>
              <input name="premium" type="number" step="0.01" value={form.premium} onChange={handleChange} required className="input-field" />
            </div>
            <div>
              <label className="block text-[14px] mb-1.5" style={{ color: 'var(--crm-text-secondary)' }}>Commission rate</label>
              <input name="commission_rate" type="number" step="0.01" value={form.commission_rate} onChange={handleChange} className="input-field" />
            </div>
            <div>
              <label className="block text-[14px] mb-1.5" style={{ color: 'var(--crm-text-secondary)' }}>First year ($)</label>
            ...
            <div>
              <label className="block text-[14px] mb-1.5" style={{ color: 'var(--crm-text-secondary)' }}>Renewal rate</label>
              <input name="renewal_rate" type="number" step="0.01" value={form.renewal_rate} onChange={handleChange} className="input-field" />
            </div>
          </div>
          <div>
            <label className="block text-[14px] mb-2" style={{ color: 'var(--crm-text-secondary)' }}>Renewal months</label>
          ...
          <div className="flex gap-3">
            <button type="submit" className="px-5 py-2.5 rounded-xl text-[14px] font-semibold" style={{ background: 'var(--crm-accent)', color: '#fff' }}>{editing ? 'Update' : 'Add'}</button>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b" style={{ borderColor: 'var(--crm-border)' }}>
              ...
              <th className="text-right px-5 py-4 text-[14px] font-medium" style={{ color: 'var(--crm-text-secondary)' }}>Premium</th>
              <th className="text-right px-5 py-4 text-[14px] font-medium" style={{ color: 'var(--crm-text-secondary)' }}>Rate</th>
              <th className="text-right px-5 py-4 text-[14px] font-medium" style={{ color: 'var(--crm-text-secondary)' }}>First year</th>
              <th className="text-right px-5 py-4 text-[14px] font-medium" style={{ color: 'var(--crm-text-secondary)' }}>Renewal</th>
              <th className="text-left px-5 py-4 text-[14px] font-medium" style={{ color: 'var(--crm-text-secondary)' }}>Months</th>
          ...
          <tbody>
            {policies.map((p) => (
              <tr key={p.id} className="border-b hover:bg-white/[0.02]" style={{ borderColor: 'var(--crm-border)' }}>
                <td className="px-5 py-4 text-[15px] font-semibold">{p.client_name}</td>
                <td className="px-5 py-4 text-right text-[14px]">${(p.premium || 0).toLocaleString()}/yr</td>
                <td className="px-5 py-4 text-right text-[14px]">{((p.commission_rate || 0) * 100).toFixed(0)}%</td>
                <td className="px-5 py-4 text-right text-[14px] font-semibold" style={{ color: 'var(--crm-accent)' }}>${(p.first_year_amount || 0).toLocaleString()}</td>
                <td className="px-5 py-4 text-right text-[14px]">{((p.renewal_rate || 0) * 100).toFixed(1)}%</td>
                <td className="px-5 py-4 text-[13px]" style={{ color: 'var(--crm-text-muted)' }}>{(p.renewal_months || []).map((m) => MONTHS[m]).join(', ') || '—'}</td>
                <td className="px-5 py-4">
                  <span className="text-[13px] px-3 py-1 rounded-lg font-medium" style={{
                    background: p.status === 'active' ? 'rgba(30,144,255,0.12)' : p.status === 'pending' ? 'rgba(255,255,255,0.05)' : 'rgba(239,68,68,0.1)',
                ...
                <td className="px-5 py-4 text-right">
                  <button onClick={() => { setForm({ ...p }); setEditing(p.id); setShowForm(true); }} className="text-[13px] mr-3" style={{ color: 'var(--crm-text-muted)' }}>Edit</button>
                  <button onClick={() => handleDelete(p.id)} className="text-[13px]" style={{ color: 'var(--crm-negative)' }}>Del</button>
                </td>
              </tr>
            ))}
          </tbody>
